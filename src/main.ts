...
function renderSkills(): void {
  ...
  resumeData.skills.forEach((skillCategory: SkillCategory) => {
    ...
    card.innerHTML = `
      ...
      ${skillCategory.logos && skillCategory.logos.length > 0 ? `
        <div class="skill-logos">
          ${skillCategory.logos.map(logo => `<img src="${logo}" class="skill-logo-icon" alt="Tool" />`).join('')}
        </div>
      ` : ''}
    `;
      ...
      <div class="timeline-card">
        ...
        ${exp.images && exp.images.length > 0 ? `
          <div class="timeline-images">
            ${exp.images.map(img => `<img src="${img}" alt="Work sample" class="timeline-img" loading="lazy" />`).join('')}
          </div>
        ` : ''}
      </div>
    `;
